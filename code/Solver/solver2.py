...
class Sudoku_Solver_2(Sudoku_Solver):
	def __init__(self):
		super(Sudoku_Solver_2, self).__init__()
		self._log = True
		self._threads = []
	...
	def find_all(self, code, row, col):
		for i in range(1, 10):
			if self.is_okay(code, row, col, i):
				...
				new_row = next_posn // 9
				new_col = next_posn % 9
				self.start_find_all(code, new_row, new_col)


	def start_find_all(self, code, row, col):
		x = threading.Thread(target=self.find_all, args=(code, row, col,))
